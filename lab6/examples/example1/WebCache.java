...
public class WebCache {
	...
	public String requestURL(String url) {
		...
		if (cache.containsKey(url)) { // daca deja avem in cache continutul
			WebCacheEntry e = (WebCacheEntry)cache.get(url);
			System.out.println("am ales "+ url +" cu accesari"+e.accesari);
		...
		System.out.println("Am ales "+ url +" cu accesari 0");
			...
			while (sizeOnDisk > maxSizeOnDisk) { // cat timp am depasit mai stergem intrari
				...
				for (Enumeration en = cache.keys(); en.hasMoreElements(); ) {
					...
					if (wce == null) wce = e;
						...
						if (wce.accesari < e.accesari) wce = e; // alegem pagina cea mai veche accesata pentru stergere (LRU)
					}
				}
				if (wce == null) break;
				// stergem respectiva intrare din cache
				System.out.println("\nam eliminat intrarea "+ wce.getURL()+" cu accesari"+wce.accesari+"\n");
